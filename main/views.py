...
def petal(request, petal_id):
    ...
    news = News.objects.filter(project__subscriptions_petals__id=petal_id).order_by('-create_date')
    context = {'petal': petal,
               'news': news}
...
class MusicianProjectView(View):
    ...
    def post(self, request, project_id):
        ...
        if request.is_ajax():
            if project.subscriptions_petals.filter(id=request.user.id).exists():
                Petal.objects.get(id=request.user.id).subscriptions.remove(project)
                sub_users = [i.first_name for i in project.subscriptions_petals.all()]
                context_ajax = {'subs': 'Subscribe',
                                'sub_users': sub_users}
            else:
                Petal.objects.get(id=request.user.id).subscriptions.add(project)
                sub_users = [i.first_name for i in project.subscriptions_petals.all()]
...
class Album(View):
    def get(self, request, project_id, album_id):
        album = MusicianProject.objects.get(id=project_id).musicalbum_set.get(id=album_id)
        context = {'album': album}
